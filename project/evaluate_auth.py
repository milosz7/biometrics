...
def extract_user_from_path(img_path):
...
def load_images(path):
    ...
    paths = [os.path.join(path, img_path) for img_path in paths if img_path.endswith(".jpg")]
...
def evaluate_impostors(model, impostors_dataset, embeddings, threshold):
    results, probs = [], []
...
def evaluate_authorized(model, users_dataset, row_to_user_map, embeddings, threshold):
    results = []
    probs = []
...
def evaluate_auth(config):
    ...
    y_true = [0.0] * get_num_imgs(impostors_dataset) + [1.0] * get_num_imgs(users_dataset)

    threshold = config.THRESHOLD
    y_pred_impostors, y_probs_impostors = evaluate_impostors(model, impostors_dataset, embeddings, threshold)
    y_pred_users, y_probs_users = evaluate_authorized(model, users_dataset, row_to_user_map, embeddings, threshold)
